...
public:
    TwoLayerInitializer(plint nx_,plint ny_, bool topLayer_)
        :nx(nx_),
         ny(ny_),
         topLayer(topLayer_)
    { }
    ...
    virtual void execute(plint iX, plint iY, T rand_val, Cell<T,Descriptor>& cell) const {
        ...
        bool check=(plb::util::sqr(iX-nx/2) + plb::util::sqr(iY-ny/4) <= (ny/4)*(ny/4));
        if((topLayer&&check)||(!topLayer&&!check) )    
        {
                          ...
                          T force )
{
    ...
    applyIndexed(heavyFluid, Box2D(0, nx-1, 0, ny-1),
                 new TwoLayerInitializer<T,DESCRIPTOR>(nx,ny, true) );
    // Initialize bottom layer.
    applyIndexed(lightFluid, Box2D(0, nx-1, 0, ny-1),
                 new TwoLayerInitializer<T,DESCRIPTOR>(nx,ny, false) );
...
int main(int argc, char *argv[])
{
    ...
    heavyFluid.periodicity().toggle(0,false);
    lightFluid.periodicity().toggle(0,false);
